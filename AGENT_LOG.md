...
**Skills roster expanded, one renamed.** User pushed back correctly on the previous `financial-analysis` skill: it was entirely P&L/ratio analysis, not general financial-analyst capability, despite the name implying broader scope. Renamed to `financial-statement-analysis` (same content) rather than broadening it - budgeting/forecasting and marketing analysis are real, separate gaps (user's own recurring work: budgeting systems, marketing project analysis, P&L-vs-KPI work) and should be their own skills when built, not folded into one overloaded file.

**Real research this round, not just reacting to conversation mentions:** audited the user's actual Workspace/Desktop project folders directly rather than only what came up in chat. Found a major domain entirely missed in the first pass - sustained trading-strategy/backtesting work under the Kabroda brand (`KTBB_app`/`KTBB_app_v2`, `PA Pivots`, `BackTest_Kabroda_System`, plus the user's own `Trading Knowledge` library project). A `trading-strategy-analysis` skill (Sharpe ratio, max drawdown, Calmar ratio, walk-forward validation as the field's actual gold standard, Deflated Sharpe Ratio for selection-bias correction) is proposed but **not yet built** - flagged as a good next candidate, ties directly into the user's own tested-vs-theory discipline in `Trading Knowledge`.

**Two new skills built and shipped** (both global, both sourced from real published standards, not invented):
- `system-architecture` - C4 model (Simon Brown). Settle context+container before writing code; explicitly warns against over-architecting a simple project, not just under-architecting a complex one.
- `code-review` - Google's Engineering Practices Code Review Developer Guide. Design/functionality checked before complexity/tests/naming/style, so a review can't be all naming nitpicks with no comment on whether the approach is right.

**Conceptual finding worth keeping:** there are two different kinds of skill, not one spectrum of "how global is too global." *Process skills* (`project-scaffolding`, `system-architecture`, `code-review`) apply to almost any project regardless of domain - they map to phases of a generic 8-phase build lifecycle (requirements → planning → architecture → implementation → code review → testing → refactoring → deployment; found as a real pattern in a community skill catalog's `agent-squad` role set, not invented). *Domain skills* (`financial-statement-analysis`, and the still-unbuilt `budgeting-forecasting`/`marketing-analysis`/`trading-strategy-analysis`) are specific to what a project is *about*. Global scope is correct for a domain skill as long as the domain itself recurs across multiple projects - the earlier Back40 Provisions mixup was about content tied to *one specific project's own identity* landing in global scope, not about domain skills in general being risky to make global.
