...
This repoints your Antigravity Start Menu shortcut at `launch-antigravity.bat`, which runs a sub-second patch check (`ensure-patched.ps1`) before actually opening Antigravity - so the moment you reopen it after an update, it silently re-patches first if needed. No admin required (it only edits your own user Start Menu shortcut and a per-user registry key, not machine-wide settings), and it works identically whether you're on the admin (`deploy.ps1`) or portable/no-admin (`portable-deploy.ps1`) install - `ensure-patched.ps1` detects which one is present.

It also registers a second, independent check that runs silently once per login (a per-user `HKCU` Run key, no visible window) regardless of how Antigravity actually gets opened. This matters for a real failure mode, confirmed in practice: an Antigravity update/reinstall can reset the Start Menu shortcut back to pointing directly at `Antigravity.exe`, undoing the repoint above. Since the patch check used to only run *through* that shortcut, a reset shortcut meant nothing ever ran again to notice or fix it - the self-healing silently stopped working until someone noticed the mod itself was gone. The per-login check doesn't depend on the shortcut being correct, so it catches and repairs exactly this case (both the shortcut and the patch) within one login cycle instead of indefinitely.

An alternative timer-based scheduled task (`install-auto-repair-task.ps1`) also exists in the repo, but isn't installed by default - Task Scheduler launching PowerShell causes a brief visible console flash every interval, which is disruptive during real use. Only reach for it if you launch Antigravity some way that bypasses both the Start Menu shortcut and a normal login.
