...
function proxyToGoogle(req: http.IncomingMessage, res: http.ServerResponse, reqBody: Buffer): void {
  ...
  const proxyReq = https.request(parsedUrl, options, (proxyRes) => {
    ...
    if (shouldBufferAndModify) {
      ...
      proxyRes.on('end', () => {
        ...
        if (!res.writableEnded) res.end(modifiedBuffer);
      });
    } else {
      if (!res.headersSent) {
  ...
  res.on('close', () => {
    if (!res.writableEnded) {
      log.warn('[Proxy] Client closed connection early to Google Forwarder');
...
function handleCustomModelRequest(
    ...
    if (isStream) {
      if (apiRes.statusCode! >= 400) {
        ...
        apiRes.on('end', () => {
          ...
          if (!res.writableEnded) res.end(errorBody);
        });
        return;
      }
      ...
      let body = '';
      apiRes.on('data', (chunk: Buffer) => {
        if (!res.writableEnded) body += chunk;
      });
      apiRes.on('end', () => {
        if (res.writableEnded || res.headersSent) return;
  ...
  res.on('close', () => {
    if (!res.writableEnded) {
      log.warn(`[Proxy] Client closed connection early for ${model.name}. Aborting payload.`);
...
function handleGetAvailableModelsProxy(
  ...
  const lsReq = client.request(options, (lsRes) => {
    ...
    lsRes.on('end', () => {
      ...
      if (!res.writableEnded) res.end(modifiedBuf);
    });
  ...
  res.on('close', () => {
    if (!res.writableEnded) {
      if (!lsReq.destroyed) lsReq.destroy();
    }
  });
...
function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
  ...
  req.on('end', () => {
    ...
    if (req.url!.startsWith('/GetAvailableModels')) {
      ...
      if (!res.writableEnded) res.end(JSON.stringify({ error: 'Missing ls parameter' }));
      return;
    }
    ...
    if (req.url!.includes('/v1internal:fetchAvailableModels')) {
      ...
      const googleReq = https.request(parsedUrl, fwdOptions, (googleRes) => {
        googleReq.setTimeout(30_000, () => {
          ...
          if (!res.headersSent) {
            ...
            res.writeHead(200, { 'Content-Type': 'application/json' });
            if (!res.writableEnded) res.end(JSON.stringify({ models: mappedCustom }));
          }
        });
        ...
        googleRes.on('end', () => {
          ...
          try {
            ...
            if (!res.writableEnded) res.end(JSON.stringify(googleJson));
          } catch (err) {
      ...
      res.on('close', () => {
        if (!res.writableEnded && !googleReq.destroyed) googleReq.destroy();
      });
      ...
      try {
        ...
        if (modelName) {
          ...
          if (matchedCustomModel) {
            ...
            const toolGroups = (actualGeminiBody.tools || []) as { functionDeclarations?: { name: string }[] }[];
    ...
    if (req.method === 'POST' && (isGenerate || isStandardStream)) {
      ...
      if (matchedCustomModel) {
        try {
          ...
          if (!res.headersSent) res.writeHead(400, { 'Content-Type': 'application/json' });
          if (!res.writableEnded) res.end(JSON.stringify({ error: { message: 'Invalid JSON request body' } }));
          return;
        }
      }
    }
